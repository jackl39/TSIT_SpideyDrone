...
class droneRepublisher:
    """
    Starts off takeoff and landing sequency. 
    Publishes data to /tello/cmd_vel for movement
    Subscribes to /tello/odom for position
    """
    def __init__(self):
    ...
    def set_speeds(self, x_dot, y_dot, z_dot, x_ang, y_ang, z_ang):
        command = Twist()
...
if __name__ == '__main__':
    ...
    x, y, z, = drone.get_pose()
    set_y = y + 1
    while not rospy.is_shutdown():
        _, curr_y, _ = drone.get_pose()
        while curr_y < set_y:
            drone.set_speeds(0, 0.3, 0, 0, 0, 0)
